...
public class LinkedListTabulatedFunction implements TabulatedFunction, Serializable {
    ...
    public LinkedListTabulatedFunction(FunctionPoint[] points){
        ...
        head.prev = head;
        this.pointsCount = 0;
        addNodeToTail(new FunctionPoint(points[0]));
    ...
    public void setPoint(int index, FunctionPoint point) {
        ...
        if(index > 0){
            ...
            if(x <= leftX){
                throw new IllegalArgumentException("значение Х новой точки должно быть больше Х соседней левой точки");
            }
        }
        if(index < pointsCount -1){
            double rightX = getNodeByIndex(index+1).point.getX();
            if(x>=rightX){
                throw new IllegalArgumentException("значение Х новой точки должно быть больше Х соседней правой точки");
            }
        }
